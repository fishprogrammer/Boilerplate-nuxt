    ...
    <div class="page-shell">
        <div class="page-card">
            <div class="mb-6 flex flex-col gap-3 border-b border-border/50 pb-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-xl font-semibold text-text-primary">ایجاد نقش جدید</h1>
            ...
            <CreateRoleFormSkeleton v-if="isLoadingPermissions" />

            <div v-else-if="permissionsLoadError" class="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-300">
                {{ permissionsLoadError }}
            </div>

            <form v-else @submit.prevent="submitCreate" class="space-y-5">
                <div>
                ...
                <div class="flex flex-wrap gap-2">
                    <button
                        ...
                        class="btn-action-sm"
                    >
                        {{ isSaving ? 'در حال ذخیره...' : 'ایجاد نقش' }}
                    </button>
                    <RouterLink
                        :to="{ name: 'roles' }"
                        class="btn-muted-sm"
...
function applyApiErrors(err: unknown) {
    ...
    saveError.value = getApiErrorMessage(err, 'خطا در ایجاد نقش')
}
...
async function fetchAllPermissions() {
    ...
    try {
        ...
        while (page <= totalPages) {
            ...
            if (!parsed) {
                permissionsLoadError.value = 'دریافت لیست دسترسی‌ها با خطا مواجه شد.'
                permissions.value = []
                return
            }
        ...
        permissionsLoadError.value = getApiErrorMessage(err, 'خطا در دریافت لیست دسترسی‌ها')
        permissions.value = []
    } finally {
...
async function submitCreate() {
    ...
    try {
        ...
        const created = parseCreateRoleResponse(response)
        saveSuccess.value = `نقش «${created?.name || roleName.value}» با موفقیت ایجاد شد.`
